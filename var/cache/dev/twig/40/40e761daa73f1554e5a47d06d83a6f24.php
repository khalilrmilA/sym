...
\t<script>
\t\t// Define the regex patterns outside of the functions
\t\tconst primaryKeyRegex = /^[0-9]+\$/;
...
\t\t  const isValid = primaryKeyRegex.test(privateKeyInput.value.trim());
\t\t  privateKeyInput.classList.toggle('is-valid', isValid);
\t\t  privateKeyInput.classList.toggle('is-invalid', !isValid);
\t\t}
\t\t
\t\tfunction validateSubject() {
\t\t  const subjectInput = document.getElementById('reclamation_subject');
\t\t  const isValid = subjectRegex.test(subjectInput.value.trim());
\t\t  subjectInput.classList.toggle('is-valid', isValid);
\t\t  subjectInput.classList.toggle('is-invalid', !isValid);
\t\t}
\t\t
\t\tfunction validateDescription() {
\t\t  const descriptionInput = document.getElementById('reclamation_description');
\t\t  const isValid = descriptionRegex.test(descriptionInput.value.trim());
\t\t  descriptionInput.classList.toggle('is-valid', isValid);
\t\t  descriptionInput.classList.toggle('is-invalid', !isValid);
\t\t}
\t\t
\t\t// Add event listeners
\t\tdocument.getElementById('reclamation_privateKey').addEventListener('input', validatePrivateKey);
\t\tdocument.getElementById('reclamation_subject').addEventListener('input', validateSubject);
\t\tdocument.getElementById('reclamation_description').addEventListener('input', validateDescription);
\t\t
...
\t\t\tif (form.checkValidity() && !document.querySelectorAll('.is-invalid').length) {
...
\t\t\t  .then(data => {
\t\t\t\tif (data.success) {
\t\t\t\t  Swal.fire('Success!', 'Your reclamation has been submitted.', 'success');
...
\t\t\t\t\tconst input = document.querySelector(`[name=\"reclamation[\${key}]\"]`);
...
\t\t\t\tSwal.fire('Error!', 'There was an error submitting your reclamation.', 'error');
\t\t\t  });
\t\t\t} else {
\t\t\t  // If the form is not valid, trigger the validation UI
\t\t\t  form.reportValidity();
\t\t\t}
\t\t  });
\t\t});
